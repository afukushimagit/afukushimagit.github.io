...
![export_anim_noparams](images/export/export_anim_noparams.png)

TIFFフォーマットの連番が付いた画像ファイルが保存される．

### 書式2

画像のフォーマットやファイル名を変更したい場合は，こちらの書式を用いる．

```java
saveFrame( "ファイル名.拡張子" )
```

ファイル名には複数の**ハッシュマーク**を記述する必要がある．
...
ハッシュマークが**フレーム番号**に置き換えられる．
ハッシュマークの`#`の**数でフレーム番号の最大桁数**を指定することができる．
...
![export_anim_selected_time](images/export/export_anim_selected_time.png)

`frameCount`

プログラムを実行してから，現在の描画フレームの番号を取得するシステム変数．

`frameCount`を使って以下のようにif文を用いることで，指定した２つのフレーム番号間だけ出力を行うことができる．
...
void draw()
{
  ...
  if( frameCount >= 20 && frameCount <= 40 )	// framecount が20以上かつ40以下
  {
    saveFrame( "screen-####.png" );
  }
}
...
```java
save( "フォルダ名/ファイル名.拡張子" )
```

```java
saveFrame( "フォルダ名/ファイル名.拡張子" )
```
...
![export_anim_directory](images/export/export_anim_directory.png)
...
2. `size()`の記述を変更する．
   ...
   ```java
   size( 横のサイズ, 縦のサイズ, PDF, “ファイル名.pdf” );
   ```
...
3. 　プログラム末尾に`exit();`を追加．
    Activeモードを使用中ならsetup()ブロックの末尾に記述する．
...
1. 「ファイル」 > 「アプリケーションとしてエクスポート」を選択 
   ...
   ![export_app_step01](images/export/export_app_step01.png)

2. オプションを選んでエクスポート

   ![export_app_step02](images/export/export_app_step02.png)

   - フルスクリーンを止めたい時は，「フルスクリーン」 > 「プレゼンテーションモード」のチェックを**外す**．
   - Mac OS X 用に出力する場合，「プラットフォーム」 > 「Max OS X」にチェックを**入れる**．
...
3. 出力フォルダ
   ...
   ![export_app_step03](images/export/export_app_step03.png)

   動作させるOSが64bitか32bitかで，使用するフォルダは異なる．
   **昨今のOSはほぼ64bit**なので，こちらを用いることを前提とする．
   「application.windows64」フォルダの中身が全て必要であるので，配布などを行う場合，**このフォルダごと配布**する．（フォルダのリネームは可）
